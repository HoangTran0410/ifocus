...
import { Volume2, VolumeX, Youtube, ExternalLink, History, Play, Music as MusicIcon } from 'lucide-react';
import { SoundTrack } from '../types';

interface AudioControllerProps {
  sounds: SoundTrack[];
  setSounds: (sounds: SoundTrack[]) => void;
  youtubeUrl: string;
  ...
  return (
    <div className="flex flex-col h-full text-white">
      <div className="flex border-b border-white/10 mb-4">
        <button 
          className={`flex-1 py-3 text-sm font-medium ${activeTab === 'sounds' ? 'border-b-2 border-white text-white' : 'text-white/50 hover:text-white'}`}
          onClick={() => setActiveTab('sounds')}
        >
          Ambience
        </button>
        <button 
          className={`flex-1 py-3 text-sm font-medium ${activeTab === 'media' ? 'border-b-2 border-white text-white' : 'text-white/50 hover:text-white'}`}
        ...
        {activeTab === 'sounds' && (
          <div className="grid grid-cols-2 gap-3 pb-4">
            {sounds.map(sound => (
              ...
              >
                <div className="text-3xl mb-2 filter drop-shadow-md">{sound.emoji}</div>
                ...
                >
                  <input
                    ...
                    className={`w-full h-1 rounded-lg appearance-none cursor-pointer transition-colors ${
                      sound.isPlaying ? 'bg-white/40 accent-white' : 'bg-white/10 accent-white/50'
                    }`}
                  />
                </div>
              </div>
        ...
        {activeTab === 'media' && (
          <div className="space-y-6">
            <form onSubmit={handleUrlSubmit} className="space-y-2">
              <label className="text-xs text-white/70 uppercase tracking-wider">Media URL</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/70 flex items-center gap-2">
                    {embedInfo.type === 'spotify' ? 'Spotify' : embedInfo.type === 'apple' ? 'Apple Music' : 'YouTube'} Player
                  </span>
                  <button 
                ...
                {showYoutube && (
                  <div className={`relative bg-black rounded-xl overflow-hidden shadow-lg border border-white/10 ${embedInfo.type === 'youtube' ? 'pt-[56.25%]' : 'h-40'}`}>
                    <iframe
                   ...
                   {youtubeHistory.map((url, idx) => (
                     ...
                     >
                       <div className="w-8 h-6 bg-white/10 rounded flex items-center justify-center shrink-0">
                         {url.includes('spotify') ? <MusicIcon size={12} className="text-green-400" /> : 
                          url.includes('apple') ? <MusicIcon size={12} className="text-pink-500" /> : 
            ...
            <div className="text-xs text-white/30 leading-relaxed">
              Supports YouTube videos, Spotify tracks/playlists, and Apple Music albums. Paste the link and we'll handle the embed.
            </div>
          </div>
        )}
      </div>
