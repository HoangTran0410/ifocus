...
import { Menu, X, Music, CheckSquare, Edit3, Image as ImageIcon, Settings, Maximize2, Minimize2 } from 'lucide-react';
...
import { Scene, SoundTrack, Task, Note, TimerMode, EffectType } from './types';
...
function App() {
  // Application State
  const [currentScene, setCurrentScene] = useLocalStorage<Scene>('zen_scene', DEFAULT_SCENES[0]);
  const [currentEffect, setCurrentEffect] = useLocalStorage<EffectType>('zen_effect', 'none');
  const [sounds, setSounds] = useLocalStorage<SoundTrack[]>('zen_sounds', DEFAULT_SOUNDS);
  ...
  const [youtubeHistory, setYoutubeHistory] = useLocalStorage<string[]>('zen_yt_history', []);
  ...
  const addToHistory = (url: string) => {
    ...
    const newHistory = [url, ...youtubeHistory.filter(u => u !== url)].slice(0, 5);
      ...
      <div className="relative z-10 w-full h-full flex flex-col pointer-events-none">
        ...
        <div className="flex justify-between items-center p-6 pointer-events-auto">
          <div className="flex items-center gap-3">
             ...
             <span className="text-white font-semibold tracking-wide text-lg shadow-black drop-shadow-md">ZenFocus</span>
          </div>
        ...
        <div className="flex-1 flex items-center justify-center p-4 pointer-events-auto">
          <div className={`transition-all duration-500 ${activePanel !== 'none' ? 'lg:-translate-x-32' : ''}`}>
          ...
          <div className={`transition-all duration-500 ${activePanel !== 'none' ? 'lg:-translate-x-32' : ''}`}>
            <div className="flex gap-4 p-2 bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl">
              <DockButton 
                ...
                onClick={() => setActivePanel(activePanel === 'scenes' ? 'none' : 'scenes')}
              />
              <DockButton 
                icon={<Music size={20} />} 
                label="Sounds" 
                isActive={activePanel === 'audio'}
                onClick={() => setActivePanel(activePanel === 'audio' ? 'none' : 'audio')}
              />
...
// Helper Subcomponent
const DockButton = ({ icon, label, isActive, onClick }: { icon: React.ReactNode, label: string, isActive: boolean, onClick: () => void }) => (
  <button
    onClick={onClick}
