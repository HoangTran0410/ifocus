import{e as m,d as v,f as h,b as g,i as _,j as y,k as b,l as S,F as w}from"./utils-E6iCwlZp.js";const x=`
...
`,t={program:null,uniforms:{},time:0,mouseX:0,mouseY:0};function P({ctx:s,canvas:o,data:a,performanceMode:l=!1,beatIntensity:n=0,bass:i=0,mid:f=0,high:r=0}){if(!m(o.width,o.height))return;const p=v(),e=y();if(!e)return;if(!t.program){if(t.program=h(e,w,x),!t.program)return;t.uniforms=g(e,t.program)}if(_(),!t.program)return;t.time+=l?.012:.016,t.mouseX+=2e-4,t.mouseY=.5+Math.sin(t.time*.05)*.1;const u=a.reduce((c,d)=>c+d,0)/a.length;e.viewport(0,0,e.drawingBufferWidth,e.drawingBufferHeight),e.clearColor(0,0,0,0),e.clear(e.COLOR_BUFFER_BIT),e.useProgram(t.program),e.uniform1f(t.uniforms.u_time,t.time),e.uniform1f(t.uniforms.u_intensity,u),e.uniform1f(t.uniforms.u_beatIntensity,n),e.uniform1f(t.uniforms.u_bass,i),e.uniform1f(t.uniforms.u_mid,f),e.uniform1f(t.uniforms.u_high,r),e.uniform2f(t.uniforms.u_resolution,o.width,o.height),e.uniform2f(t.uniforms.u_mouse,t.mouseX%1,t.mouseY),e.enable(e.BLEND),e.blendFunc(e.SRC_ALPHA,e.ONE_MINUS_SRC_ALPHA),b(t.program),S(p,s,o)}function k(){t.program=null,t.uniforms={}}export{k as cleanup,P as default};
