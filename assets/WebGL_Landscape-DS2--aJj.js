import{e as m,d,f as h,b as y,i as p,j as _,k as g,l as b,F as C}from"./utils-E6iCwlZp.js";const S=`
...
`,t={program:null,uniforms:{},time:0};function w({ctx:o,canvas:s,data:i,performanceMode:c=!1,beatIntensity:a=0,bass:r=0,mid:f=0,high:u=0}){if(!m(s.width,s.height))return;const l=d(),e=_();if(!e)return;if(!t.program){if(t.program=h(e,C,S),!t.program)return;t.uniforms=y(e,t.program)}if(p(),!t.program)return;t.time+=c?.012:.016;const v=i.reduce((n,x)=>n+x,0)/i.length;e.viewport(0,0,e.drawingBufferWidth,e.drawingBufferHeight),e.clearColor(0,0,0,0),e.clear(e.COLOR_BUFFER_BIT),e.useProgram(t.program),e.uniform1f(t.uniforms.u_time,t.time),e.uniform1f(t.uniforms.u_intensity,v),e.uniform1f(t.uniforms.u_beatIntensity,a),e.uniform1f(t.uniforms.u_bass,r),e.uniform1f(t.uniforms.u_mid,f),e.uniform1f(t.uniforms.u_high,u),e.uniform2f(t.uniforms.u_resolution,s.width,s.height),e.enable(e.BLEND),e.blendFunc(e.SRC_ALPHA,e.ONE_MINUS_SRC_ALPHA),g(t.program),b(l,o,s)}function O(){t.program=null,t.uniforms={}}export{O as cleanup,w as default};
