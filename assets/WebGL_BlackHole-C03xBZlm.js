import{e as h,d as g,f as m,b as v,i as _,j as R,k as j,l as b,F as S}from"./utils-E6iCwlZp.js";const y=`
...
`,t={program:null,uniforms:{},time:0};function k({ctx:u,canvas:i,data:n,performanceMode:w=!1,beatIntensity:l=0,bass:d=0}){if(!h(i.width,i.height))return;const c=g(),e=R();if(!e)return;if(!t.program){if(t.program=m(e,S,y),!t.program)return;t.uniforms=v(e,t.program)}if(_(),!t.program)return;t.time+=.016*(1+l*.5);const p=n.reduce((o,r)=>o+r,0)/n.length,f=new Float32Array(8),a=Math.floor(n.length/8);for(let o=0;o<8;o++){let r=0;for(let s=0;s<a;s++)r+=n[o*a+s]||0;f[o]=r/a}e.viewport(0,0,e.drawingBufferWidth,e.drawingBufferHeight),e.clearColor(0,0,0,0),e.clear(e.COLOR_BUFFER_BIT),e.useProgram(t.program),e.uniform1f(t.uniforms.u_time,t.time),e.uniform1f(t.uniforms.u_intensity,p),e.uniform1f(t.uniforms.u_beatIntensity,l),e.uniform1f(t.uniforms.u_bass,d),e.uniform2f(t.uniforms.u_resolution,i.width,i.height),e.uniform1fv(t.uniforms["u_frequencies[0]"],f),e.enable(e.BLEND),e.blendFunc(e.SRC_ALPHA,e.ONE_MINUS_SRC_ALPHA),j(t.program),b(c,u,i)}function D(){t.program=null,t.uniforms={}}export{D as cleanup,k as default};
